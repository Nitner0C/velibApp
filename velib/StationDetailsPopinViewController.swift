...
class StationDetailsPopinViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func fillData(station : Station?) {
        if let s = station {
            if (s.distanceToUser != -1.0) {
                titleLabel.text = "Infos Station (\(Int(s.distanceToUser))m)"
            } else {
                titleLabel.text = "Infos Station"
            }
            bikeNumberLabel.text = "\(s.bike_stands) stands à vélo (\(s.available_bikes) dispo)"
            collectionView.reloadData()
            delegate?.moveMapToCoordinates(s.lat, lng: s.lng)
        }
    }
    ...
    func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
        ...
        if let image : UIImageView = cell.contentView.subviews[0] as? UIImageView {
            if let s = station {
                image.image = UIImage(named: (indexPath.row < s.available_bikes ? "bike_green" : "bike_red"))
            }
        }
        return cell
    }
